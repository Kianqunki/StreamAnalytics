...
def __checktype( typename, a, b ):
    ...
    if typename == "n":
        return (isinstance(a, numpy.int64) or np.integer(a) or type(a) is int or type(a) is float or type(a) is numpy.int64) and (type(b) is int or type(b) is float or type(b) is numpy.int64 or np.integer(b) or isinstance(b, numpy.int64))
    # For unknown types
    return False
...
def __distance( a, b, info, datatype, on_both_none = 0.0, on_none = 1.0 ):
    ...
    # TODO: enable this: assert __checktype( datatype[0], a, b )
...
## @completed, incremental
def mixeddistance( a, b, SKELETON, on_both_none = 0.0, on_none = 1.0 ):
